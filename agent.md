...
- **AWS Free Tier Production Deployment Infrastructure:**
  ...
  - **CI/CD Extended Pipeline Timeout & Sequential Docker Builds:** Configured `appleboy/ssh-action` with `timeout: 60m, command_timeout: 60m` and sequential service compilation in `deploy.yml`, preventing AWS `t3.micro` CPU throttling during Next.js and PyTorch compilation.
